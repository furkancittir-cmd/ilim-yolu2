...
const STORAGE_KEY = "ilim-yolu-v3";
...
وَأَشْهَدُ أَنَّ مُحَمَّدًا عَبْدُهُ وَرَسُولُهُ`,
    ...
    meaning: `Bütün selamlar, ibadetler ve güzel şeyler Allah içindir. Ey Nebi, Allah’ın selamı, rahmeti ve bereketi senin üzerine olsun. Selam bizim üzerimize ve Allah’ın صالح kullarının üzerine olsun. Şahitlik ederim ki Allah’tan başka ilah yoktur. Ve şahitlik ederim ki Muhammed O’nun kulu ve elçisidir.`,
  },
  { id: "salli", name: "Allâhumme Salli", arabic: `اللّٰهُمَّ صَلِّ عَلَىٰ سَيِّدِنَا مُحَمَّدٍ
وَعَلَىٰ آلِ سَيِّدِنَا مُحَمَّدٍ`, translit: `Allâhümme salli alâ seyyidinâ Muhammed. Ve alâ âli seyyidinâ Muhammed.`, meaning: `Allah’ım! Efendimiz Muhammed’e ve onun âline salât eyle.` },
  { id: "barik", name: "Allâhumme Bârik", arabic: `اللّٰهُمَّ بَارِكْ عَلَىٰ سَيِّدِنَا مُحَمَّدٍ
وَعَلَىٰ آلِ سَيِّدِنَا مُحَمَّدٍ`, translit: `Allâhümme bârik alâ seyyidinâ Muhammed. Ve alâ âli seyyidinâ Muhammed.`, meaning: `Allah’ım! Efendimiz Muhammed’e ve onun âline bereket ver.` },
  ...
  { id: "kunut_hanefi_dua", name: "Kunut (Hanefî)", arabic: `اللّٰهُمَّ إِنَّا نَسْتَعِينُكَ وَنَسْتَغْفِرُكَ...`, translit: `Allâhümme innâ nesteînüke ve nestağfirüke...`, meaning: `Allah’ım! Senden yardım ister, bağışlanma diler ve Sana dayanırız.` },
  { id: "kunut_shafii_dua", name: "Kunut (Şâfiî)", arabic: `اللّٰهُمَّ اهْدِنِي فِيمَنْ هَدَيْتَ...`, translit: `Allâhümmehdinî fîmen hedeyt...`, meaning: `Allah’ım! Hidayet verdiklerin arasında bana da hidayet ver.` },
];
...
const zikrData = [
  ...
  { id: "salavat", name: "Salavat-ı Şerif", arabic: "اللّٰهُمَّ صَلِّ عَلَىٰ مُحَمَّدٍ", translit: "Allâhümme salli alâ Muhammed", meaning: "Allah’ım Muhammed’e salât eyle.", defaultTarget: 33 },
...
function App() {
  ...
  useEffect(() => {
    setState((s) => {
      const all = Object.values(s.prayerDone).every(Boolean);
      return { ...s, prayerStreak: all ? s.prayerStreak : 0 };
    });
  }, [state.prayerDone]);
  ...
  function logout() {
    setState((s) => ({ ...DEFAULT_STATE, username: s.username, auth: false }));
  }
  ...
  function currentTesbihatSteps() {
    const steps = [
      ...
      ...(state.tesbihatType === "long"
        ...
        : []),
      { key: "ayetelkursi", label: "Ayetel Kürsi", title: "اللّٰهُ لَا إِلٰهَ إِلَّا هُوَ الْحَيُّ الْقَيُّومُ ...", translit: "Allâhu lâ ilâhe illâ hüve'l-hayyü'l-kayyûm...", meaning: "Ayetel Kürsi tam okunur.", count: 1, showLongText: true },
  ...
  const nav = [["home", Home, "Home"], ["sureler", BookOpen, "Sureler"], ["tesbihat", Layers3, "Tesbihat"], ["zikir", Sparkles, "Zikir"], ["oyun", Play, "Oyun"], ["profil", User, "Profil"]];
  const selectedZikrCount = state.zikrCounts?.[state.zikrSelected] || 0;

  if (!state.auth) {
    return (
      <div className="min-h-screen bg-[radial-gradient(circle_at_top,_#f8f7f0,_#e6f2ea_60%,_#d8e6d8)] p-4 text-slate-900">
        <div className="mx-auto flex min-h-[calc(100vh-2rem)] max-w-6xl items-center justify-center">
          <div className="grid w-full gap-6 lg:grid-cols-2">
            <div className="rounded-[2rem] border border-white/60 bg-white/90 p-8 shadow-2xl backdrop-blur">
              <div className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-4 py-2 text-emerald-700"><ShieldCheck className="h-4 w-4" /> İlim Yolu</div>
              <h1 className="mt-5 text-4xl font-black tracking-tight text-emerald-950">Namazı, sureleri, tesbihatı ve zikirleri tek yerde öğren.</h1>
              <p className="mt-4 max-w-xl text-slate-600">Yeni başlayan biri için sade, büyük metinli ve adım adım öğrenme odaklı bir ibadet yardımcısı.</p>
              <div className="mt-6 grid gap-3 sm:grid-cols-3">
                <Stat label="XP" value={state.xp} icon={Zap} />
                <Stat label="Seviye" value={state.level} icon={Crown} />
                <Stat label="Rozet" value={3} icon={Medal} />
              </div>
            </div>
            <div className="rounded-[2rem] border border-emerald-200 bg-[#faf7f0] p-8 shadow-2xl">
              <div className="flex items-center gap-2 text-lg font-bold text-emerald-900"><LogIn className="h-5 w-5" /> Giriş / Kayıt</div>
              <div className="mt-6 space-y-4">
                <Field label="Kullanıcı adı" value={state.username} onChange={(v) => setState((s) => ({ ...s, username: v }))} placeholder="kullaniciadi" />
                <Field label="Şifre" value="••••••••" onChange={() => {}} placeholder="••••••••" type="password" />
                <button onClick={login} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-700 px-4 py-3 font-semibold text-white shadow-lg shadow-emerald-200 hover:bg-emerald-800"><LogIn className="h-4 w-4" /> Sisteme giriş yap</button>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#faf7f0] text-slate-900">
      ...
      <nav className="fixed inset-x-0 bottom-0 border-t border-emerald-100 bg-white/95 backdrop-blur">
    ...
    <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
      <div className="rounded-[2rem] border border-emerald-100 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <h3 className="text-xl font-bold text-emerald-950">Sureler</h3>
            <p className="text-sm text-slate-600">Seçtiğin sure hemen sağ panelde açılır.</p>
          </div>
        ...
        <FullCard title={selectedSurah.name} subtitle={`${selectedSurah.arabicTitle} · ${selectedSurah.verses} ayet`}>
          <div className="grid gap-4 lg:grid-cols-[0.3fr_0.7fr]">
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4 text-right">
              ...
              <div className="text-[12px] leading-6 text-slate-700" dir="rtl" style={{ lineHeight: 2.05 }}>{selectedSurah.arabic}</div>
            </div>
    ...
    <div className="space-y-4">
      <div className="sticky top-4 rounded-[2rem] border border-emerald-100 bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h3 className="text-xl font-bold text-emerald-950">Zikir</h3>
            <p className="text-sm text-slate-600">Önce zikri seç, üstteki sayaç sabit kalsın.</p>
          </div>
          <div className="rounded-2xl bg-emerald-50 px-3 py-2 text-sm font-semibold text-emerald-900">Hedef {target}</div>
        </div>
        <div className="mt-4 rounded-[2rem] border border-emerald-100 bg-emerald-50 p-5 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-sm font-semibold text-emerald-800">Seçili zikrin sayacı</div>
              <div className="text-2xl font-black text-emerald-950">{selectedZikr.name}</div>
            </div>
            <div className="rounded-2xl bg-white px-3 py-2 text-sm font-semibold text-emerald-900">Bu zikir: {selectedZikrCount}</div>
          </div>
          <div className="mt-4 rounded-3xl bg-white p-4 text-center">
            <div className="text-right text-3xl leading-10 text-emerald-950" dir="rtl">{selectedZikr.arabic}</div>
            <div className="mt-3 text-lg font-semibold text-slate-900">{selectedZikr.translit}</div>
            <div className="mt-2 text-sm text-slate-600">{selectedZikr.meaning}</div>
          </div>
          <div className="mt-4 flex gap-2">
            <button onClick={addZikr} className="flex-1 rounded-2xl bg-emerald-700 px-4 py-3 font-semibold text-white">+1</button>
            <button onClick={resetZikr} className="rounded-2xl border px-4 py-3 font-semibold"><RotateCcw className="h-4 w-4" /></button>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <p className="text-sm text-slate-600">Aşağıdan seç, üst panel anında değişsin.</p>
...
function ProfileView({ state, logout }) {
  return (
    <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
      <div className="rounded-[2rem] border border-emerald-100 bg-white p-4 shadow-sm">
        <h3 className="text-xl font-bold text-emerald-950">Profil</h3>
